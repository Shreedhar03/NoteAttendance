...
const Navbar = (props) => {
  ...
  const markFirstLecture = async () => {
    ...
    const rollNos = Object.keys(existingData.data()['C']).length !== 0 ? existingData.data()['C'].presentCount : [0]
    const outOf = Object.keys(existingData.data()['C']).length !== 0 ? existingData.data()['C'].outOf : [0]
    const record = {
    ...
    try {
      if (!(existingData?.data()['C']['Dated'] == today))
  ...
  const markFirstLecture1 = async () => {
    const existingData = await getDoc(docRef)
    // await updateDoc(docRef,{'A.name':"K"})
    // console.log("toBeEdited",existingData.data()[formValues.div])
    // await updateDoc(docRef,{'A':{}})
